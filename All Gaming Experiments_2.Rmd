...
Standardizing actual game performance for each game
```{r}
data_gaming <- transform(data_gaming, Score_norm = ave(as.numeric(Score), Gameplayed, FUN = scale))
data_rank <- transform(data_gaming, Score_norm = ave(as.numeric(Score), Gameplayed, FUN = scale))
data_faces <- transform(data_gaming, Score_norm = ave(as.numeric(Score), Gameplayed, FUN = scale))
```

```{r}
Figure1a <- ggplot(data_gaming, aes(x = Score_norm, y = Selfscore, color = Sex, shape = Sex, linetype = Sex)) + 
  geom_point(size = 4) + 
...
```{r}
Perf <- lm(Score_norm ~   Sex * VorNV * Age, data = data_gaming)
...
##MANOVA
```{r}
MANOVA_gaming<- manova(cbind(MV1Total, SGAAverage, SOIRTOTAL) ~ Age + Sex + VorNV + Averagefrequencyviolence + Selfscore + Score_norm +  VorNV*Selfscore + VorNV*Sex + Age*Sex + Sex*Selfscore, data=data_gaming)
...
##MANOVA
```{r}
MANOVA_rank<- manova(cbind(MV1Total, SGAAverage, SOIRTOTAL) ~ Age + Sex + VorNV + Averagefrequencyviolence + Selfscore + Rank + VorNV*Selfscore + VorNV*Sex + Age*Sex + VorNV*Rank + Sex*Selfscore, data=data_rank)
...
##MANOVA
```{r}
MANOVA_face<- manova(cbind(MV1Total, SGAAverage, SOIRTOTAL) ~ Age + Sex + VorNV + Averagefrequencyviolence + Selfscore + VorNV*Selfscore + VorNV*Sex + Age*Sex + Sex*Selfscore, data=data_faces)
